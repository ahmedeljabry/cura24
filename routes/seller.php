...
Route::group(['prefix'=>'seller','middleware'=>['auth','inactiveuser','BuyerCheck','userEmailVerify','setlang','globalVariable']],function(){
    ...
    Route::get('/orders/{status?}','Frontend\SellerController@sellerOrders')->name('seller.orders');
    // job all order list
    Route::get('/job-orders','Frontend\SellerController@sellerJobOrders')->name('seller.job.orders');
    Route::post('/check-otp-required', 'Frontend\SellerController@checkOtpRequired')->name('seller.check.otp.required');
    Route::post('/send-job-complete-confirm-otp', 'Frontend\SellerController@sendJobComplteConfirmOtp')
        ->name('seller.send.job.complete.confirm.otp');
