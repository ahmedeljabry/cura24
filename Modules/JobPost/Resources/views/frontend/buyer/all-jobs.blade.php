...
<h4 class="dashboard_jobPost__author__title text_max_show">
    @if(!empty($data->slug))
        <a target="_blank" href="{{ route('job.post.details', ['slug' => $data->slug]) }}">
            {{ $data->title }}
        </a>
    @else
        {{ $data->title }}
    @endif
</h4>                                                    <h6 class="dashboard_jobPost__author__price mt-2">
