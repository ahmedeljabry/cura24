...
class BuySubscriptionController extends Controller
{
    ...
    public function cancelSubscriptionHistory(Request $request)
    {
        $request->validate([
            'history_id' => 'required|exists:subscription_histories,id',
        ]);

        $history = SubscriptionHistory::find($request->history_id);

        if (!$history) {
            return response()->json(['type' => 'error', 'msg' => 'Subscription history not found.'], 404);
        }

        // Only allow cancellation if status is not already paid
        if ($history->payment_status === 'completed') {
            return response()->json(['type' => 'error', 'msg' => 'Cannot cancel a completed payment.'], 400);
        }

        $history->payment_status = 'cancelled';
        $history->save();

        return response()->json(['type' => 'success', 'msg' => 'Subscription cancelled successfully.']);
    }
