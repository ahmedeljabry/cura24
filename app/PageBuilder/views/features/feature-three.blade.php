                        ...
                        <div class="award_icons">
                            <a href="javascript:void(0)" class="wishlist-icon" data-service-id="{{ $service->id }}"
                               title="Add to Wishlist">
                                ...
                                @else
                                    <i class="fa-regular fa-heart fa-lg text-red"></i> {{-- darker outline to show better --}}
                                @endif
                            </a>
                        </div>
                    </div>
...
<script>
    $(document).on('click', '.wishlist-icon', function (e) {
        ...
        let isFavourite = button.find('i').hasClass('fa-solid'); // Use Font Awesome
        let url = isFavourite
            ? '{{ route("service.list.buyer.remove-favourite.service") }}'
            : '{{ route("service.list.buyer.add-favourite.service", ["service_id" => ":id"]) }}'.replace(':id', serviceId);
        let method = 'POST';
        let data = isFavourite ? {service_id: serviceId} : {};
        ...
        $.ajax({
            ...
            success: function (response) {
                // Only toggle icon for successful actions
                if (response.type === 'success') {
                    $('.wishlist-icon[data-service-id="' + serviceId + '"] i').each(function () {
                        if (isFavourite) {
                            $(this).removeClass('fa-solid fa-heart text-danger').addClass('fa-regular fa-heart text-red');
                        } else {
                            $(this).removeClass('fa-regular fa-heart text-red').addClass('fa-solid fa-heart text-danger');
                        }
                    });
                }
                toastr[response.type](response.msg || response.message || 'Action completed');
            },
            error: function (xhr) {
                let message = xhr.responseJSON?.msg || xhr.responseJSON?.message || 'Something went wrong.';
