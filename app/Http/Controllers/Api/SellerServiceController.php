...
class SellerServiceController extends Controller
{
    ...
    public function addService(Request $request, GoogleTranslateService $translator)
    {
        if ($request->isMethod('post')) {
            ...
            $service->slug = createSlug($translatedData['title'] ?? $request->title, 'service');
    ...
    public function updateService(Request $request, GoogleTranslateService $translator)
    {
        if ($request->isMethod('post')) {
            $request->validate([
                'category_id' => 'required',
                'title' => 'required|max:191|unique:services,title,'.$request->service_id,
                'description' => 'required|min:150',
            ]);

            $translatedData = $this->translateFlatServicePayload($translator, $request->all());
            $this->validateTranslatedServiceTitle($translatedData['title'] ?? $request->title, (int) $request->service_id);
    ...
    public function addServiceAttributesByID(Request $request, GoogleTranslateService $translator, $id=null)
    {
        ...
        if($request->isMethod('post')) {
            ...
            $new_include_services = $this->decodeAndTranslateApiCollection(
                $translator,
                $data['all_include_service'] ?? null,
                ['*.*.include_service_title' => 'text']
            );
            $new_additional_services = $this->decodeAndTranslateApiCollection(
                $translator,
                $data['all_additional_service'] ?? null,
                ['*.*.additional_service_title' => 'text']
            );
            $new_benifits_services = $this->decodeAndTranslateApiCollection(
                $translator,
                $data['service_benifits'] ?? null,
                ['*.*.benifits' => 'text']
            );
            $new_faqs_services = $this->decodeAndTranslateApiCollection(
                $translator,
                $data['online_service_faqs'] ?? null,
            ...
            if(!empty($new_additional_services)) {
                foreach ($new_additional_services as $value) {
                    foreach ($value as $service){
                        $all_additional_service[] = $service +
                            [
                                'seller_id' => Auth::guard('sanctum')->user()->id,
            ...
            if(!empty($new_benifits_services)) {
                foreach ($new_benifits_services as $value) {
                    foreach ($value as $service){
                        $all_benifits_service[] = $service +
                            [
                                'seller_id' => Auth::guard('sanctum')->user()->id,
                                'benifits' => 0,
                            ];
            ...
            if(!empty($new_faqs_services)){
                foreach ($new_faqs_services as $value) {
                    foreach ($value as $service){
                        $online_service_faqs[] = $service +
                            [
                                'seller_id' => Auth::guard('sanctum')->user()->id,
                                'title' => '',
                                'description' => '',
    ...
    public function updateServiceAttributesByID(Request $request, GoogleTranslateService $translator, $id=null)
    {
        $get_service = Service::where('id',$id)->where('seller_id',Auth::guard('sanctum')->user()->id)->first();
    ...
    private function decodeAndTranslateApiCollection(
        ...
        return $translator->translatePayload($decodedPayload, $paths);
    }
