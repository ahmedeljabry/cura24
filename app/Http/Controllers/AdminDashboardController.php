...
class AdminDashboardController extends Controller
{
    ...
    public function update_dashboard_variant_buyer_seller(Request $request)
    {
        ...
        $this->validate($request, [
            'start_week_from' => 'required|in:0,1,2,3,4,5,6',
            'timezone' => 'required|in:' . implode(',', timezone_identifiers_list()),
            'time_format' => 'required|in:12,24',
            'date_format' => ['required', function ($attribute, $value, $fail) use ($validDateFormats) {
                if (!in_array($value, $validDateFormats, true)) {
                    $fail(__('Invalid date format selected.'));
                }
            }],
        ], [
            'start_week_from.in' => __('Invalid day selected.'),
            'timezone.in' => __('Invalid time zone selected.'),
            'time_format.in' => __('Invalid time format selected.'),
            'date_format.in' => __('Invalid date format selected.'),
        ]);

        update_static_option('start_week_from', $request->start_week_from);
        update_static_option('timezone', $request->timezone);
        update_static_option('time_format', $request->time_format);
        update_static_option('date_format', $request->date_format);
