...
//    }
    ...
    private function update_database($extra_service_id, $transaction_id)
    {
        ...
        if (!$extraService) {
            \Log::warning("update_database: ExtraService not found for ID {$extra_service_id}");
            return;
        }
        ...
        // Fetch related order
        $orderDetails = $extraService->order; // assuming relation exists: ExtraService belongsTo Order
        if (!$orderDetails) {
            \Log::warning("update_database: No order found for ExtraService ID {$extra_service_id}");
            return;
        }
        ...
        if ($seller) {
            $pushTitle = __('Extra Service Confirmed');
            $pushBody  = __('Buyer has confirmed and paid for extra service on Order #:id', [
                'id' => $orderDetails->id
            ]);
            ...
            $extraData = [
                ...
                'type'     => 'ExtraServiceConfirmed',
            ];

            send_push_notification(
                $seller,
                $orderDetails->id,
                'ExtraServiceConfirmed',
                $pushBody,
                $pushTitle,
                $extraData,
                $orderDetails,
        ...
        // ✅ (Optional) Send push notification to buyer as confirmation
//        $buyer = $orderDetails->buyer;
//        if ($buyer) {
//            $pushTitle = __('Extra Service Payment Successful');
//            $pushBody  = __('You have successfully paid for extra service on Order #:id', [
//                'id' => $orderDetails->id
//            ]);
//
//            $extraData = [
