...
class ServiceListController extends Controller
{
    ...
    public function createOrder(Request $request)
    {
        ...
        $buyer_name = optional(Auth::guard('web')->user())->name ?? $request->name;
        ...
        $notification_title = __('You have a new order.');
        // seller buyer notification
        $seller->notify(new OrderNotification($notification_title, $last_order_id, $request->service_id, $request->seller_id, $buyer_id, $order_message));
        ...
        if(moduleExists('Wallet')){
            if ($request->selected_payment_gateway === 'wallet') {
                    ...
                    if($wallet_balance->balance >= $order_details->total){
                        ...
                        try {
                            ...
                            send_push_notification(
                                ...
                                $last_order_id,
                                "Order",
                                sprintf('You have received a order from %s (Order: %s)', $buyer_name, $service->title),
                ...
                if ($request->input('request_source') === 'admin') {
                    return redirect()->route('admin.add.order')
                             ->with('success', 'Order #' . $new_order_id . ' created successfully.');
                }
    ...
    public function categoryServices($slug = null)
    {
        ...
        if(!is_null($category)){
            ...
            $cities = ServiceCity::select('id', 'service_city')
                ->orderBy('service_city', 'asc')
                ->get();
           Service::query()->with(['reviews', 'serviceCity']);
            ...
            if (!empty(request()->get('location')) && is_numeric(request()->get('location'))) {
                $service_quyery->where('service_city_id', request()->get('location'));
            }
    ...
    public function subCategoryServices($slug = null)
    {
       ...
       $cities = ServiceCity::select('id', 'service_city')
        ...
        // Build service query
        $service_quyery = Service::query()->with(['reviews', 'serviceCity']);
