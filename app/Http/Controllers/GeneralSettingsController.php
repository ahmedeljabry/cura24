...
class GeneralSettingsController extends Controller
{
    ...
    public function update_basic_settings(Request $request)
    {
        $this->validate($request, [
            ...
            'site_google_captcha_version' => 'nullable|string|in:v2,v3',
...
//        }
   ...
   public function update_smtp_settings(Request $request){
    ...
    public function update_payment_settings(Request $request)
    {
        ...
        $env_val['MERCHANT_EMAIL'] = $request->paystack_merchant_email ?: env('MERCHANT_EMAIL', '');

        $env_val['MOLLIE_KEY'] = $request->mollie_public_key ?: env('MOLLIE_KEY', '');

        $env_val['FLW_PUBLIC_KEY'] = $request->flw_public_key ?: env('FLW_PUBLIC_KEY', '');
        $env_val['FLW_SECRET_KEY'] = $request->flw_secret_key ?: env('FLW_SECRET_KEY', '');
        $env_val['FLW_SECRET_HASH'] = $request->flw_secret_hash ?: env('FLW_SECRET_HASH', '');

        $env_val['RAZORPAY_API_KEY'] = $request->razorpay_api_key ?: env('RAZORPAY_API_KEY', '');
        $env_val['RAZORPAY_API_SECRET'] = $request->razorpay_api_secret ?: env('RAZORPAY_API_SECRET', '');

        $env_val['STRIPE_PUBLIC_KEY'] = $request->stripe_public_key ?: env('STRIPE_PUBLIC_KEY', '');
        $env_val['STRIPE_SECRET_KEY'] = $request->stripe_secret_key ?: env('STRIPE_SECRET_KEY', '');
    ...
    public function pusher_settings(Request $request)
    {
        if($request->isMethod('post')){
            ...
            $data = $this->validate($request, [
                'broadcaster_option' => 'required|in:pusher,reverb',
                'pusher_app_id' => $request->broadcaster_option === 'pusher' ? 'required' : 'nullable',
                'pusher_app_key' => $request->broadcaster_option === 'pusher' ? 'required' : 'nullable',
                'pusher_app_secret' => $request->broadcaster_option === 'pusher' ? 'required' : 'nullable',
                'pusher_app_cluster' => $request->broadcaster_option === 'pusher' ? 'required' : 'nullable',
            ...
            // Update .env file
            $envData = [
                'BROADCAST_DRIVER' => $request->broadcaster_option,
            ];

            // Only update Pusher credentials in .env if broadcaster_option is pusher
            if ($request->broadcaster_option === 'pusher') {
                $envData = array_merge($envData, [
                    'PUSHER_APP_ID' => trim($request->pusher_app_id),
                    'PUSHER_APP_KEY' => trim($request->pusher_app_key),
                    'PUSHER_APP_SECRET' => trim($request->pusher_app_secret),
                    'PUSHER_APP_CLUSTER' => trim($request->pusher_app_cluster),
                ]);
            }
        ...
        $selectedBroadcaster = get_static_option('broadcaster_option') ?? 'pusher';
        return view('backend.general-settings.pusher-settings', compact('selectedBroadcaster'));
    }

    public function push_notification_setting(Request $request)
    {
        if($request->isMethod('post')){
        ...
        return view('backend.general-settings.push-notification-settings',compact('firebaseFileExists'));
    }
