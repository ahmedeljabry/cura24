...
use Modules\WhatsAppBookingSystem\Entities\WhatsAppDetails;
...
{
    if(moduleExists('WhatsAppBookingSystem') && !empty(getenv('DB_DATABASE')))
    {
        $option_name = $key;
        $value = \Illuminate\Support\Facades\Cache::remember($option_name, 600, function () use($option_name) {
            try {
                return \Modules\WhatsAppBookingSystem\Entities\WhatsAppDetails::where('option_name', $option_name)->first();
            }catch (\Exception $e){
                return null;
            }
        });
...
function update_whatsapp_option($key, $value)
{
    try {
        if (
            moduleExists('WhatsAppBookingSystem') &&
            app()->runningInConsole() === false && // skip during install/migrate
            Schema::hasTable('whats_app_details')
        ) {
            $existing = WhatsAppDetails::where('option_name', $key)->first();

            if (!$existing) {
                WhatsAppDetails::create([
                    'option_name'  => $key,
                    'option_value' => $value
                ]);
            } else {
        ...
        // swallow any DB errors during installation
        return false;
    }
...
function get_user_lang_direction()
{
    ...
    $user_direction = \App\Language::where('slug', $current_lang)->first();
...
function render_page_meta_data($blog_post){
    ...
    $description= cleanText($blog_post?->blog_content);

    $meta_title =  $blog_post?->meta_data?->meta_title ?: $title;
    $site_tags = $blog_post?->meta_data?->meta_tags ?: $title;
    $site_description =  $blog_post?->meta_data?->meta_description ?: $description;

    $facebook_meta_tags = $blog_post?->meta_data?->facebook_meta_tags ?: $meta_title;
    $facebook_meta_description =  $blog_post?->meta_data?->facebook_meta_description ?: $site_description;
    ...
    $twitter_meta_tags = $blog_post?->meta_data?->twitter_meta_tags ?: $meta_title;
    $twitter_meta_description =  $blog_post?->meta_data?->twitter_meta_description ?: $site_description;
...
function render_page_meta_data_for_service($service_details){
    ...
    $facebook_meta_image =  get_attachment_image_by_id($service_details->metaData?->facebook_meta_image ?: $service_details?->image)['img_url'] ?? '';
    $facebook_meta_image_alt =  get_attachment_image_by_id($service_details->metaData?->facebook_meta_image ?: $service_details?->image)['img_alt'] ?? '';
    ...
    $twitter_meta_image =  get_attachment_image_by_id($service_details->metaData?->twitter_meta_image ?: $service_details?->image)['img_url'] ?? '';
...
function render_page_meta_data_for_category($category){
    ...
    $site_tags = $category->metaData?->meta_tags ?: $title;
    $site_description =  $category->metaData?->meta_description ?: $description;

    $facebook_meta_tags = $category->metaData?->facebook_meta_tags ?: $meta_title;
    $facebook_meta_description =  $category->metaData?->facebook_meta_description ?: $site_description;
    $facebook_meta_image =  get_attachment_image_by_id($category->metaData?->facebook_meta_image ?: $category?->image)['img_url'] ?? '';
    $facebook_meta_image_alt =  get_attachment_image_by_id($category->metaData?->facebook_meta_image ?: $category?->image)['img_alt'] ?? '';

    $twitter_meta_tags = $category->metaData?->twitter_meta_tags ?: $meta_title;
    $twitter_meta_description =  $category->metaData?->twitter_meta_description ?: $site_description;
    $twitter_meta_image =  get_attachment_image_by_id($category->metaData?->twitter_meta_image ?: $category?->image)['img_url'] ?? '';
...
function render_page_meta_data_for_subcategory($subcategory){
    ...
    $description= cleanText($subcategory->description);
    $meta_title =  $subcategory->metaData?->meta_title ?: $title;
    $site_tags = $subcategory->metaData?->meta_tags ?: $title;
    $site_description =  $subcategory->metaData?->meta_description ?: $description;

    $facebook_meta_tags = $subcategory->metaData?->facebook_meta_tags ?: $meta_title;
    $facebook_meta_description =  $subcategory->metaData?->facebook_meta_description ?: $site_description;
    ...
    $twitter_meta_tags = $subcategory->metaData?->twitter_meta_tags ?: $meta_title;
    $twitter_meta_description =  $subcategory->metaData?->twitter_meta_description ?: $site_description;
...
function render_page_meta_data_for_child_category($child_category){
    ...
    $description= cleanText($child_category->description);
    $meta_title =  $child_category->metaData?->meta_title ?: $title;
    $site_tags = $child_category->metaData?->meta_tags ?: $title;
    $site_description =  $child_category->metaData?->meta_description ?: $description;

    $facebook_meta_tags = $child_category->metaData?->facebook_meta_tags ?: $meta_title;
    $facebook_meta_description =  $child_category->metaData?->facebook_meta_description ?: $site_description;
    ...
    $twitter_meta_tags = $child_category->metaData?->twitter_meta_tags ?: $meta_title;
    $twitter_meta_description =  $child_category->metaData?->twitter_meta_description ?: $site_description;
...
function build_notification_description(string $type, array $extraData): string
{
    switch ($type) {
        case 'Job':
            return sprintf(
                "Seller: %s%s",
                $extraData['seller_name'] ?? 'Unknown Seller',
                isset($extraData['expected_salary'])
                    ? " | Expected Salary: {$extraData['expected_salary']}"
                    : ''
            );

        case 'JobStatus':
            return sprintf(
                "Job ID: %s | Status changed from '%s' to '%s'",
                $extraData['job_id'] ?? 'N/A',
                $extraData['old_status'] ?? 'Unknown',
                $extraData['new_status'] ?? 'Unknown'
            );

        case 'Order':
            return sprintf(
                "Buyer: %s | Service: %s",
                $extraData['buyer_name'] ?? 'Unknown Buyer',
                $extraData['service_name'] ?? 'N/A'
            );

        case 'OrderStatus':
            return sprintf(
                "Order ID: %s | Status changed from '%s' to '%s'",
                $extraData['order_id'] ?? 'N/A',
                $extraData['old_status'] ?? 'Unknown',
                $extraData['new_status'] ?? 'Unknown'
            );

        case 'OrderCompletionRequest':
            return sprintf(
                "Order ID: %s | Seller (ID: %s) has requested completion",
                $extraData['order_id'] ?? 'N/A',
                $extraData['seller_id'] ?? 'Unknown'
            );
        ...
        case 'Chat':
            return sprintf(
                "Message from %s: %s",
                $extraData['sender_name'] ?? 'Unknown',
                $extraData['message'] ?? 'New message'
            );

        case 'Service':
            return sprintf(
                "Service: %s",
                $extraData['service_name'] ?? 'Untitled Service'
            );
...
function send_push_notification($notifiable, $identity, $type, $msg, $title, $extraData = [], $relatedData = null, $insert = false): string
{
    ...
    // Store notification in DB (polymorphic notifications table)
    $notificationId = (string) Str::uuid();
    if ($insert) {
        // Prepare notification data, including related data if provided
        $notificationData = [
            'id' => $notificationId,
            'type' => $type,
            'notifiable_type' => get_class($notifiable),
            'notifiable_id' => $notifiable->id,
            'data' => json_encode(array_merge($extraData, [
                'title' => $title,
                'order_message' => $msg,
                'related_data' => $relatedData ? $relatedData->toArray() : null,
            ])),
            'created_at' => now(),
            'updated_at' => now(),
        ];
    ...
    // Generate description using only type and extraData
    $description = build_notification_description($type, $extraData);

    // Prepare Firebase push payload
    $notificationBody = [
        ...
        'type' => $type,
        'sound' => 'default',
        'fcm_device' => '', // Keep key for Flutter parsing
        'related_data' => $relatedData ? json_encode($relatedData->toArray()) : null,
    ];

    try {
        if ($notifiable->firebase_device_token) {
            ...
            $tokens = is_array($notifiable->firebase_device_token)
                ? $notifiable->firebase_device_token
                : [$notifiable->firebase_device_token];
            $message = CloudMessage::new()->withData($notificationBody);
