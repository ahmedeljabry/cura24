                ...
                <div class="col-lg-12">
                    ...
                    <div class="tab-content active" id="tab_one">
                        @if(!empty(get_static_option('site_google_captcha_enable')))
                            @if(get_static_option('site_google_captcha_version') === 'v2')
                                <!-- Load reCAPTCHA v2 API -->
                                <script src="https://www.google.com/recaptcha/api.js" async defer></script>
                            @elseif(get_static_option('site_google_captcha_version') === 'v3')
                                ...
                                <fieldset class="fieldset-service service-area">
                                    <div class="information-all margin-top-55">
                                        ...
                                        <div class="info-service">
                                            ...
                                            <div class="single-info-service margin-top-30 service_categories_wrapper d-none">
                                                <div class="single-content">
                                                    <label class="forms-label"> {{ __('Categories of Services Provided') }}
                                                    </label>
                                                    <select name="service_categories[]" id="service_categories"
                                        ...
                                        <div class="col-md-12 my-3">
                                            @if(get_static_option('site_google_captcha_version') === 'v2')
                                                <div class="g-recaptcha"
                                                     data-sitekey="{{ get_static_option('site_google_captcha_v2_site_key') }}"></div>
                                            @elseif(get_static_option('site_google_captcha_version') === 'v3')
                    ...
                    <div class="social-login-wrapper">
                        ...
                        <div class="sin-in-with">
                            @if(get_static_option('enable_google_login'))
                                <a href="{{ route('login.google.redirect', ['type' => ($reg_type === 'buyer' ? 1 : 0), 'origin' => 'signup']) }}"
                                   id="googleLoginBtn"
                                   class="sign-in-btn">
                            ...
                            @if(get_static_option('enable_facebook_login'))
                                <a href="{{ route('login.facebook.redirect', ['type' => ($reg_type === 'buyer' ? 1 : 0), 'origin' => 'signup']) }}"
                                   id="facebookLoginBtn"
                                   class="sign-in-btn">
...
@section('scripts')
    ...
    @if(!empty(get_static_option('site_google_captcha_enable')) && get_static_option('site_google_captcha_version') === 'v3')
        <script>
            grecaptcha.ready(function() {
                grecaptcha.execute("{{ get_static_option('site_google_captcha_v3_site_key') }}", {action: 'register'}).then(function(token) {
        ...
        (function() {
            ...
            $(document).ready(function() {
               ...
               var user_type = "{{$reg_type === 'buyer' ? 1 : 0}}";
                ...
                // Show/hide category field for buyer/seller
                $(document).on('click', '.is_user_buyer', function () {
                    $('.service_categories_wrapper').addClass('d-none');
                    $('.seller_category_hide_show_req').addClass('d-none');
                    updateLoginHrefs();
                });

                $(document).on('click', '.is_user_seller', function () {
                    $('.service_categories_wrapper').removeClass('d-none');
                    $('.seller_category_hide_show_req').removeClass('d-none');
                    updateLoginHrefs();
                });

                function updateLoginHrefs() {
                    const userType = $('#get_user_type').val(); // should be 1 for buyer, 0 for seller
                    const origin = 'signup';

                    const googleLoginBtn = document.getElementById('googleLoginBtn');
                    if (googleLoginBtn) {
                        googleLoginBtn.href = `/google/redirect?type=${userType}&origin=${origin}`;
                    }

                    const facebookLoginBtn = document.getElementById('facebookLoginBtn');
                    if (facebookLoginBtn) {
                        facebookLoginBtn.href = `/facebook/redirect?type=${userType}&origin=${origin}`;
                    }
                }
