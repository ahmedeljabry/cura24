    ...
    <script>
        ...
        $(document).ready(function () {
            function initAutocomplete() {
                ...
                //Auto-fill address if seller_address is empty but lat/lng exist
                function autoFillAddressFromCoordinates() {
                    var lat = <?= $location->latitude ?? 0 ?>;
                    var lng = <?= $location->longitude ?? 0 ?>;
                    var currentAddress = "{{$location->seller_address ?? ''}}";

                    // Only auto-fill if address is empty but coordinates exist
                    if ((lat !== 0 && lng !== 0) && currentAddress.trim() === '') {
                        ...
                        geocoder.geocode({ 'location': latlng }, function (results, status) {
                            if (status === google.maps.GeocoderStatus.OK) {
                                if (results[0]) {
                                    ...
                                    // Extract more detailed address components
                                    for (var i = 0; i < results[0].address_components.length; i++) {
                                        ...
                                        if (addressComponent.types.includes('sublocality') || addressComponent.types.includes('neighborhood')) {
                                            areaName = addressComponent.long_name;
                                        }
                                    }

                                    // Create a more complete address
                                    var final_address = '';
                                    if (areaName) final_address += areaName + ', ';
                                    if (cityName) final_address += cityName + ', ';
                                    final_address += countryName;

                                    // Update the seller_address field
                                    $('#seller_address').val(final_address);

                                    // Also update the hidden input to ensure it gets saved
                                    $('input[name="seller_address"]').val(final_address);
                                }
                            }
                        });
                    }
                ...
                // Call the auto-fill function when the map is initialized
                autoFillAddressFromCoordinates();
                    ...
                    geocoder.geocode({ 'location': latlng }, function (results, status) {
                        if (status === google.maps.GeocoderStatus.OK) {
                            if (results[0]) {
                                ...
                                var final_address = '';
                                if (areaName) final_address += areaName + ', ';
                                final_address += cityName + ', ' + countryName;
                                $('#seller_address').val(final_address);
                            } else {
