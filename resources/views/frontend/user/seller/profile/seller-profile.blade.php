                ...
                <div class="modal-body">
                    <div class="custom-form">
                        <form action="{{route('seller.profile.edit')}}" method="POST">
                            ...
                            <input type="hidden" name="latitude" id="profile_latitude" value="{{ Auth::guard('web')->user()->latitude ?? '' }}">
                            <input type="hidden" name="longitude" id="profile_longitude" value="{{ Auth::guard('web')->user()->longitude ?? '' }}">
                            <input type="hidden" name="seller_address" id="profile_seller_address" value="{{ Auth::guard('web')->user()->seller_address ?? '' }}">
                            @csrf
                            <div class="row g-4">
                                <div class="col-12">
                                ...
                                <div class="col-12">
                                    <div class="single-input">
                                        <label for="categories"
                                               class="label_title__postition">{{ __('Your Categories') }} <span
                                                    class="text-danger">*</span></label>
                                        <select name="category_ids[]" id="category_ids" multiple
                                                class="form--control radius-10">
                                            @foreach($categories as $category)
                                                <option value="{{ $category->id }}"
                                                        @if(in_array($category->id, $userCategoryIds)) selected @endif>
                                                    {{ $category->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <small class="form-text text-muted">{{ __('Hold Ctrl (Cmd on Mac) to select multiple.') }}</small>
                                    </div>
                                </div>

                                <div class="col-12 mt-3">
                                    <div class="single-input">
                                        <label for="experience" class="label_title__postition">
                                            {{ __('Professional Experience') }}
                                        </label>
                                        <input type="text" name="experience" id="experience"
                                               class="form--control radius-10"
                                               value="{{ old('experience', Auth::guard('web')->user()->experience) }}"
                                               placeholder="{{ __('e.g. 5 years, 3+ years, etc.') }}">
                                        <small class="form-text text-muted">
                                            {{ __('Briefly describe your professional experience.') }}
                                        </small>
                                    </div>
                                </div>
    ...
    <script>
        (function($){
            ...
            $(document).ready(function(){
                ...
                // Function to get coordinates from selected location
                function updateCoordinatesFromLocation() {
                    const countryId = $("#country_id").val();
                    const cityId = $("#service_city").val();
                    const areaId = $("#service_area").val();

                    if (!countryId || !cityId) {
                        return;
                    }

                    // Get country, city, and area names from the selected options
                    const countryName = $('#country_id option:selected').text();
                    const cityName = $('#service_city option:selected').text();
                    const areaName = $('#service_area option:selected').text();
                    ...
                    geocoder.geocode({ 'address': address }, function(results, status) {
                        if (status === google.maps.GeocoderStatus.OK && results[0]) {
                            ...
                            $('#profile_latitude').val(location.lat());
                            $('#profile_longitude').val(location.lng());
                            $('#profile_seller_address').val(results[0].formatted_address);
                        } else {
                            // If geocoding fails, try with just city and country
                            const simpleAddress = cityName + ', ' + countryName;
                            geocoder.geocode({ 'address': simpleAddress }, function(simpleResults, simpleStatus) {
                                if (simpleStatus === google.maps.GeocoderStatus.OK && simpleResults[0]) {
                                    const location = simpleResults[0].geometry.location;

                                    $('#profile_latitude').val(location.lat());
                                    $('#profile_longitude').val(location.lng());
                                    $('#profile_seller_address').val(simpleResults[0].formatted_address);
                                }
                            });
                        }
                    });
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            if (res.status == 'success') {
                                ...
                                $("#service_area").html("<option value=''>{{__('Select Area')}}</option>");

                                // Clear coordinates when country changes
                                $('#profile_latitude').val('');
                                $('#profile_longitude').val('');
                                $('#profile_seller_address').val('');
                            }
                        }
                    })
                })
                ...
                // Update coordinates when area changes
                $(document).on('change','#service_area', function() {
                    updateCoordinatesFromLocation();
                });

                // Update coordinates before form submission
                $('#editProfile form').on('submit', function(e) {
                    // Update coordinates one more time before submission
                    updateCoordinatesFromLocation();

                    // Add a small delay to ensure coordinates are updated
                    setTimeout(function() {
