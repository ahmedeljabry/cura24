                        ...
                        <form action="{{route('admin.page.update',$page_post->id)}}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="tab-content margin-top-40">
                                ...
                                <div class="form-group col-md-12">
                                    @php
                                        $currentServicePageSlug = get_static_option('select_home_page_search_service_page_url');
                                        $isCurrentServicePage = $currentServicePageSlug == $page_post->slug;
                                    @endphp

                                    <label for="update_service_list_page">
                                        <input type="checkbox" name="update_service_list_page" id="update_service_list_page" value="1"
                                                {{ $isCurrentServicePage ? 'checked' : '' }}>
                                        <strong>{{ __('Also update Service Zone Settings') }}</strong>
                                    </label>
                                    <div class="alert alert-info mt-2" style="font-size: 13px;">
                                        @if($isCurrentServicePage)
                                            <strong>✓ {{ __('This page is currently set as the Service List Page.') }}</strong><br>
                                        @endif
                                        {{ __('When checked, the "Select the Page for home page search service list display" setting in') }}
                                        <a href="{{ route('admin.service.zone.settings') }}" target="_blank" class="font-weight-bold">
                                            {{ __('Services → Google Map Settings') }}
                                        </a>
                                        {{ __('will be updated to use this page.') }}
                                    </div>
                                </div>
                                        ...
                                        <div class="tab-content" id="pageLangTabContent">
                                            ...
                                            <div class="tab-pane fade show active" id="page-it" role="tabpanel" aria-labelledby="page-it-tab">
                                                ...
                                                <div class="form-group classic-editor-wrapper @if(!empty($page_post->page_builder_status)) d-none @endif ">
                                                    ...
                                                    <textarea name="page_content" id="page_content" class="d-none">{{ $page_post->page_content }}</textarea>
                                                </div>
                                            </div>
                                            <!-- English Tab -->
                                            <div class="tab-pane fade" id="page-en" role="tabpanel" aria-labelledby="page-en-tab">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control" name="title_en" placeholder="{{__('Title (English)')}}" id="title_en" value="{{$page_post->title_en}}">
                                                </div>
                                                <div class="form-group classic-editor-wrapper @if(!empty($page_post->page_builder_status)) d-none @endif">
                                                    <label>{{__('Content (English)')}}</label>
                                                    <textarea id="jodit-editor-en" style="height: 400px;"></textarea>
                                                    <textarea name="page_content_en" id="page_content_en" class="d-none">{{ $page_post->page_content_en }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            $(document).ready(function () {
                ...
                if ($('#jodit-editor').length && !$('#jodit-editor').hasClass('jodit-initialized')) {
                    ...
                    jodit.events.on('change', () => {
                        $('#page_content').val(jodit.getEditorValue());
                    });
