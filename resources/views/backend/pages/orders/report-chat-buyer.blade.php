...
@section('style')
    ...
    <style>
        ...
        .message-content th,
        .message-content td {
            border: 1px solid #7f8a8c !important;
            padding: 8px;
        }
        ...
        .message-content ul {
            ...
            padding-left: 1.5rem !important; /* restore indent */
                                        ...
                                        <div class="reply-message-wrap ">
                                            ...
                                            <form action="{{route('admin.order.report.chat.buyer',$report_details->id.'/'.$report_details->buyer_id)}}" method="post" enctype="multipart/form-data">
                                                ...
                                                <div class="form-group">
                                                    <label for="message" class="message_buyer_seller_text_title_color">{{__('Message')}}</label>
                                                    <div id="jodit-message-editor"></div>
                                                    <textarea name="message" id="message" class="form-control d-none" cols="30" rows="5"></textarea>
                                                </div>
                                                <div class="form-group">
    ...
    <script>
        (function($){
            ...
            $(document).ready(function(){
                let joditMessage = null;
                if ($('#jodit-message-editor').length && !$('#jodit-message-editor').hasClass('jodit-initialized')) {
                    $('#jodit-message-editor').addClass('jodit-initialized');
                    joditMessage = Jodit.make('#jodit-message-editor', {
                        height: 200,
                    ...
                    joditMessage.events.on('change', () => {
                        $('#message').val(joditMessage.getEditorValue());
                    });

                    // Set initial content if exists (for edit forms)
                    const initialContent = $('#message').val();
                    if (initialContent && initialContent.trim() !== '') {
                        joditMessage.setEditorValue(initialContent);
                    }
                }
