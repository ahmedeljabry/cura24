                        ...
                        <form action="{{route('admin.seller.order.ticket')}}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="tab-content margin-top-30">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea id="seller-order-ticket-message-editor" class="form-control" name="seller_order_ticket_message">{!! get_static_option('seller_order_ticket_message') ?? '' !!}</textarea>
                                </div>
...
@section('script')
    ...
    <script>
        $(document).ready(function () {
            // Initialize Jodit for seller order ticket message editor
            let sellerOrderTicketMessageJodit = null;
            if ($('#seller-order-ticket-message-editor').length && !$('#seller-order-ticket-message-editor').hasClass('jodit-initialized')) {
                $('#seller-order-ticket-message-editor').addClass('jodit-initialized');
                sellerOrderTicketMessageJodit = Jodit.make('#seller-order-ticket-message-editor', {
                    height: 300,
                    placeholder: '{{ __("Type Seller Order Ticket Message") }}',
                    buttons: [
